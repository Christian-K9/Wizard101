...
def try_to_enchant():
    enchanted_list = ["Epic", "Potent_trap", "Sharpened_Blade"]
    unenchanted_spells = ["Orthrus", "Feint", "Spirit_Blade"]
    try_again = False
    index = 0
    while index < len(enchanted_list):
        if (si.check_for_card(enchanted_list[index]) == True) and (si.check_for_card(unenchanted_spells[index]) == True):
            si.enchant_card(enchanted_list[index], unenchanted_spells[index])
            try_again = True
        index += 1
    if try_again == True:
        try_to_enchant()
    ...
    while(len(buffs) > 0):
        for i in buffs:
            ...
            if position != None:
                ...
                if i == "Wand_Myth_Blade" or i == "Sharpened_Blade_Enchanted_Spirit_Blade":
                    si.cast_on_yourself()
                    cast_on_player = True
                else:
                    cast_on_gladiator()
